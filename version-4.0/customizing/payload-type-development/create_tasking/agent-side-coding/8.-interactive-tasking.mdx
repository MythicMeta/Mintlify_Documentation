...
## Reserved file-editor messages

Mythic 4.0 reserves message types `100` through `199` for interactive file editing. The currently defined types are:

| Value | Name | Direction |
| --- | --- | --- |
| `100` | `FileEditorRequest` | Mythic → agent |
| `101` | `FileEditorResponse` | agent → Mythic |
| `102` | `FileEditorError` | agent → Mythic |

Their base64-decoded `data` is a JSON file-editor message, not terminal bytes. Do not feed these values to a PTY or reuse them for a payload-specific extension. See [File Editor Protocol](/version-4.0/customizing/payload-type-development/create_tasking/agent-side-coding/file-editor-protocol) for the request, snapshot, conflict, and error schemas.
