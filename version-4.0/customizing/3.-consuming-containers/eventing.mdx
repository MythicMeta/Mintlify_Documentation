...
Mythic 4.0 adds a visual workflow wizard, per-step approval and typed-input gates, scoped step API tokens, richer instance details, and an operator workflow for responding to paused steps. A step can wait for approval, input, or both before its action begins.

```mermaid
flowchart LR
    T["Trigger"] --> D["Dependencies satisfied"]
    D --> A{"Approval required?"}
    A -->|"yes"| WA["Wait for approval"]
    A -->|"no"| I{"Input required?"}
    WA --> I
    I -->|"yes"| WI["Collect typed input"]
    I -->|"no"| R["Run action"]
    WI --> R
    R --> O["Publish outputs"]
```
...
Use the workflow editor to configure common triggers and actions, then inspect the generated YAML, JSON, or TOML before production use. The source file remains the portable definition and the UI preserves it with the event group.
