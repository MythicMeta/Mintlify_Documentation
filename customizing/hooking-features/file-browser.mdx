...
This is the exact same as the `supported_ui_features = ["file_browser:list"]`and the [File Browser](/customizing/hooking-features/file-browser#tasking) section above.
...
There's nothing special here outside of normal file download processes described in the [Download](/customizing/hooking-features/download) section. When a new file is tracked within Mythic, there's a "host" field in addition to the `full_path` that is reported. This information is used to look up if there's any matching browser objects and if so, they're linked together.
