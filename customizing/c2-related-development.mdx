...
<Info>
The C2 Profile has nothing to do with the *content* of the messages that are being sent. It has no influence on the encryption or what format the agent messages are in (JSON, binary, stego, etc). If you want to control that level of granularity, you need to check out the [10. Translation Containers](/customizing/payload-type-development/translation-containers).
</Info>
<Info>
