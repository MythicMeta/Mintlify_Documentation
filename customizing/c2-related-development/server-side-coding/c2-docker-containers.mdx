...
There are a few things needed to make a C2 container. For reference on general code structure options, check out [1. Payload Type Development](/customizing/payload-type-development). The general structure is the same for Payload Types, C2 Profiles, and Translation containers.
