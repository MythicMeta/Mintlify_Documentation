...
Socks proxy capabilities are a way to tunnel other traffic through another protocol. Within Mythic, this means tunneling other proxy-aware traffic through your normal C2 traffic. Mythic specifically leverages a modified Socks5 protocol (optionally with username/password for authentication).
...
<Note>
    If you configure `Mythic/.env` to have `MYTHIC_DOCKER_NETWORKING="host"`, then Mythic's core services will use `host` networking instead of `bridge` networking. This means that ports opened are automatically shared with the host. In this case, you can use UDPAssociate SOCKS functionality as well as not having to worry about which ports are exposed by Docker.
</Note>
